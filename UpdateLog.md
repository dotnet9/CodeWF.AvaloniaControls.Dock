...
## 12.1.2.13 (2026-09-22)

- 修复 Linux X11 窗口树遍历异常时未释放子窗口指针的问题。
- 检查 `XQueryTree` 和 `XGetWindowProperty` 返回状态，并记录 X11 查询异常，避免静默失败。

## 12.1.2.12 (2026-09-22)

- 移除 Demo 中未被使用且反向转换未实现的 `BoolToThicknessConverter` 遗留代码。

## 12.1.2.11 (2026-09-22)

- 在嵌入器工厂入口校验进程路径和窗口等待参数，拒绝非正超时或轮询间隔，避免运行时立即失败或忙等。

## 12.1.2.10 (2026-09-22)

- 标记未被内置主题使用的梯形标签 Padding 转换器为过时 API，避免继续扩大无用公开面。
- 将该单向转换器的反向转换异常从 `NotImplementedException` 修正为 `NotSupportedException`。

## 12.1.2.9 (2026-09-22)

- 修复梯形文档标签边框错误闭合路径导致底边被描边的问题。
...
- 将空文档模板的反射索引绑定改为编译绑定和显式列表项转换器，消除本项目产生的 IL2026/IL3050 警告。
- CI 增加 Windows Native AOT 发布验证，确保裁剪配置不会只在普通构建中通过。

## 12.1.2.6 (2026-09-22)

- 增加 `GridDock` 布局适配器单元测试，覆盖列索引、区域重叠和可见内容判断。
- 增加 GitHub Actions CI，自动执行还原、Release 构建、测试和 NuGet 打包。
- 适配器构造阶段拒绝多个区域折叠同一列，避免布局配置延迟到运行时才失败。

## 12.1.2.5 (2026-09-22)

- 修复 `FixedToolGridDockBehavior` 停用、脱离视觉树或更换适配器后列宽无法恢复的问题。
- 移除嵌套 GridDock 定位失败时的任意布局 fallback。
- 对负数列索引、区域重叠列和重复折叠列增加构造阶段校验。

## 12.1.2.4 (2026-09-22)

- 修复原生进程嵌入在 UI 线程同步等待导致界面卡顿的问题。
- 补充 Windows/Linux 嵌入失败、脱离视觉树和应用退出时的进程与原生资源清理。
- 使用程序基目录定位 Demo 自身可执行文件，避免工作目录变化导致嵌入失败。

## 12.1.2.3 (2026-09-20)

- 🚀[新增]-恢复多目标支持：NuGet 包统一使用 `net8.0;net10.0;net11.0`，并发布新版本。
...
- Document 面板由覆盖式圆角描边改为真实圆角容器，统一裁剪 TabStrip、文档内容和 MDI 内容。
- Document 活动/非活动边框改为单一边界状态切换，避免矩形内容穿出圆角和交叉处分隔线断裂。
- Tool 标题页签移除上游主题遗留的最小宽度，避免短标题出现过长的选中底线。
- 新增 Document 独立边框厚度资源，使主文档画布可以只保留圆角裁剪而不继承 Tool 卡片描边。
